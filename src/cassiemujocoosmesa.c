 ...
 * Private functions
 ******************************************************************************/
static bool load_mujoco_library2()
{
    // Buffer for paths
    char buf[4096 + 1024];
    // Get home directory
...
bool cassie_mujoco_init2(const char *file_input)
{
...
// #endif
    ...
    ctx = OSMesaCreateContextExt(OSMESA_RGBA, 24, 8, 8, 0);
    if( !ctx )
        mju_error_fp("OSMesa context creation failed");

    // make current
    if( !OSMesaMakeCurrent(ctx, buffer, GL_UNSIGNED_BYTE, 320, 240) )
        mju_error_fp("OSMesa make current failed");
...
cassie_sim_t *cassie_sim_init2(const char* modelfile, bool reinit)
{
    // Make sure MuJoCo is initialized and the model is loaded
    if (!mujoco_initialized) {
        if (!cassie_mujoco_init2(modelfile)) {
            return NULL;
        }   
    }
...
cassie_vis_t *cassie_vis_init2(cassie_sim_t* c, const char* modelfile) {
    ...
    depth_raw = (float*)malloc(sizeof(float)*width*height);
...
void cassie_vis_close2(cassie_vis_t *v)
{
    // if (!glfw_initialized || !v || !v->window)
    //     return;
